...
(function($opal) {
  ...
  $opal.add_stubs(['$attr_accessor', '$settings', '$config', '$rustSrcPath', '$racer', '$getPath', '$project', '$create_temp', '$lambda', '$getText', '$write', '$process_env_vars', '$racerBinPath', '$+', '$parse_single', '$<<', '$sort', '$compact', '$call', '$new', '$getActiveEditor', '$workspace', '$Native', '$getCursor', '$getBufferRow', '$getBuffer', '$getBufferColumn', '$getTextInRange', '$-', '$completions', '$to_n', '$match', '$respond_to?', '$first', '$captures', '$file', '$raise']);
  ;
  return (function($base) {
    ...
    (function($base, $super) {
      ...
      def.atom = def.rust_src = def.settings = def.project_path = def.editor = nil;
      ...
      def.$initialize = function(atom) {
        ...
        self.atom = atom;
        return self.settings = self.atom.$config().$settings();
      };

      def.$process_env_vars = function() {
        var $a, self = this;

        self.rust_src = ((($a = self.rust_src) !== false && $a !== nil) ? $a : self.settings.$racer().$rustSrcPath());
      ...
      def.$completions = function(editor, row, col, cb) {
        ...
        self.editor = editor;
        self.candidates = [];
        return self.$create_temp(($a = ($b = self).$lambda, $a._p = (TMP_1 = function(tempfile){var self = TMP_1._s || this, $a, $b, TMP_2, $c, TMP_3, $d, rust_src = nil, project_path = nil, racer_bin = nil, command = nil, args = nil, stdout = nil, myexit = nil, buffer = nil;
          if (self.editor == null) self.editor = nil;
          if (self.tempfile == null) self.tempfile = nil;
          if (self.text == null) self.text = nil;
          if (self.settings == null) self.settings = nil;
...
if (tempfile == null) tempfile = nil;
        self.tempfile = tempfile;
          ...
          $a = $opal.to_ary(self.$process_env_vars()), rust_src = ($a[0] == null ? nil : $a[0]), project_path = ($a[1] == null ? nil : $a[1]);
          process.env.RUST_SRC_PATH = "/Users/ericwest/installs/rust/src/:/Users/ericwest/code/rust";
          racer_bin = self.settings.$racer().$racerBinPath();
          command = "" + (racer_bin);
